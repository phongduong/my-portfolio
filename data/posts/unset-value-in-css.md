...
In the `div` element, the paragraph will be red. But I set the paragraph's `color` to `yellow`. After that, I reset the `color` property. Because the paragraph is inherited `color`'s value from its parent element, `div`. So that